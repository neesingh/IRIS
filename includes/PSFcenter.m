...
% Marsack's test cases since none of them contained prismatic coefficients.
    ...
    if nargin<3 | isempty(showit), showit = 0; end
    if nargin<2 | isempty(threshold), threshold=0.8; end

    [maxvalue,r,c]=max2D(psf); % find the pixel with peak intensity
    ...
    %show_msg(['Number of pixels used to compute PSFcenter = ',num2str(sum(mask(:)))])

    colno = center(1); rowno = center(2); % corrected code 15Apr07
    if showit
        figure; % display the PSF with center marked
        subplot(1,2,1)
        imagesc(psf), colormap gray, axis xy, axis image
        title('PSFcenter: unmasked PSF')
        subplot(1,2,2)
        imagesc(psf.*mask), colormap gray, axis xy, axis image
        title('PSFcenter: masked PSF')
        hold on; 
        plot(colno,rowno,'r+')	%mark centroid with red plus symbol
        hold off
    end
